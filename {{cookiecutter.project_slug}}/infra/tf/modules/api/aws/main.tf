...
resource "aws_route53_record" "{{cookiecutter.project_slug}}_api" {
  name    = aws_api_gateway_domain_name.{{cookiecutter.project_slug}}_api_domain_name.domain_name
  type    = "A"
  zone_id = data.aws_route53_zone.api_zone.id

  alias {
    evaluate_target_health = true
    name                   = aws_api_gateway_domain_name.{{cookiecutter.project_slug}}_api_domain_name.cloudfront_domain_name
    zone_id                = aws_api_gateway_domain_name.{{cookiecutter.project_slug}}_api_domain_name.cloudfront_zone_id
  }
}
...
resource "aws_api_gateway_stage" "{{cookiecutter.project_slug}}_api" {
...
resource "aws_api_gateway_domain_name" "{{cookiecutter.project_slug}}_api_domain_name" {
...
resource "aws_api_gateway_base_path_mapping" "example" {
  ...
  stage_name  = aws_api_gateway_stage.{{cookiecutter.project_slug}}_api.stage_name
  domain_name = aws_api_gateway_domain_name.{{cookiecutter.project_slug}}_api_domain_name.domain_name
}

resource "aws_api_gateway_rest_api" "apiLambda" {
  name        = "{{cookiecutter.project_slug}}_api_${var.environment}"
