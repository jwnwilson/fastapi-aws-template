...
def get_task_adapater(user_data: UserData = Depends(get_current_user)) -> TaskAdapter:
    queue_name = f"{{cookiecutter.project_slug}}_task_queue_{ENVIRONMENT}"
...
def get_db_adapater(user_data: UserData = Depends(get_current_user)) -> DbAdapter:
    table_name = f"{{cookiecutter.project_slug}}_task_{ENVIRONMENT}"
...
def get_task_storage_adapter(
    ...
    bucket_name = f"jwnwilson-{{cookiecutter.project_slug}}-task-{ENVIRONMENT}"
    return S3Adapter({"bucket": bucket_name}, user=user_data, upload_prefix="{{cookiecutter.project_slug}}-api")


def get_template_storage_adapter(
    user_data: UserData = Depends(get_current_user),
) -> StorageAdapter:
    bucket_name = f"jwnwilson-{{cookiecutter.project_slug}}-template-{ENVIRONMENT}"
    return S3Adapter({"bucket": bucket_name}, user=user_data, upload_prefix="{{cookiecutter.project_slug}}-api")
